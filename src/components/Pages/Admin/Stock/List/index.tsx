...
const StockListPage = memo(() => {
  ...
  const handleEdit = useCallback((current: IStock) => {
    setCurrent(current);
    setFormOpened(true);
  }, []);
  ...
  const formCallback = useCallback(
    (stock?: IStock) => {
      setFormOpened(false);
      current ? refresh() : mergeParams({ term: stock.name });
    },
  ...
  return (
    <Fragment>
      <Toolbar title='Estoque de Produtos' />

      <Card>
        <StockFormDialog opened={formOpened} stock={current} onComplete={formCallback} onCancel={formCancel} />
        ...
        <TableWrapper minWidth={500}>
          <Table>
            ...
            <TableBody>
              ...
              />
              {results.map(stock => (
                <ListItem key={stock.id} stock={stock} onEdit={handleEdit} onDeleteComplete={refresh} />
              ))}
            </TableBody>
          </Table>
        </TableWrapper>
