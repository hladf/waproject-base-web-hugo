...
const ListItem = memo((props: IProps) => {
  const { stock, onEdit, onDeleteComplete } = props;
  ...
  const [handleDelete] = useCallbackObservable(() => {
    return from(Alert.confirm(`Deseja excluir o produto ${stock.name}?`)).pipe(
      filter(ok => ok),
      tap(() => setLoading(true)),
      switchMap(() => stockService.delete(stock.id)),
      logError(),
      tap(
        () => {
          Toast.show(`${stock.name} foi removido`);
          setLoading(true);
          setDeleted(true);
          onDeleteComplete();
        },
